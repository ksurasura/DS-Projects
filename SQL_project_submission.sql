...
joindate - 2012-09-26 18:08:45
firstname - Darren
surname - Smith

SELECT m.joindate, m.firstname, m.surname
FROM Members m
INNER JOIN (

SELECT MAX( joindate ) AS joindate, firstname, surname
FROM Members
)m1 ON m.joindate = m1.joindate
...
SELECT f.name AS tenniscourtname, CONCAT( m.firstname, ' ', m.surname ) AS fullname
FROM Facilities f
JOIN Bookings b ON f.facid = b.facid
JOIN Members m ON b.memid = m.memid
WHERE f.name LIKE '%tennis%'
GROUP BY tenniscourtname, fullname
ORDER BY fullname
LIMIT 0 , 30
...
JOIN Bookings b ON f.facid = b.facid
JOIN Members m ON b.memid = m.memid
WHERE DATE( b.starttime ) LIKE '%2012-09-14%'
having cost > 30
ORDER BY 3 DESC
LIMIT 0 , 30
